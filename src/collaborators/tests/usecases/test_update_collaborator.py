...
def test_manager_can_update_collaborator_partial_success(
    ...
    use_case = UpdateCollaboratorUseCase(collaborator_repository, fixed_id_generator, auth_context)
...
def test_manager_can_update_collaborator_full_success(
    collaborator_repository, manager_alice, fixed_id_generator, john_commercial
):
    collaborator_repository.create(john_commercial)
    auth_context = AuthContext(manager_alice)
...
def test_non_manager_cannot_update_collaborator(
...
def test_update_non_existent_collaborator_raises_error(collaborator_repository, fixed_id_generator, manager_alice):
    auth_context = AuthContext(manager_alice)
    use_case = UpdateCollaboratorUseCase(collaborator_repository, fixed_id_generator, auth_context)
